...
def root():
    try:
        ...
        cursor.close()
        conn.close()
        return jsonify(message='Hello from APP talking to DB!', items=count)
...
[Unit]
Description=Flask App (3-tier demo)
After=network-online.target
Wants=network-online.target

[Service]
Type=simple
Environment=DB_HOST=${dbPrivateIp}
Environment=DB_USER=appuser
Environment=DB_PASSWORD=${mysqlAppPassword}
Environment=DB_NAME=demodb
...
runcmd:
  ...
  - bash -lc "mysql -u root << 'SQL'
ALTER USER 'root'@'localhost' IDENTIFIED WITH mysql_native_password BY '${mysqlAppPassword}';
FLUSH PRIVILEGES;
CREATE DATABASE IF NOT EXISTS demodb;
CREATE USER IF NOT EXISTS 'appuser'@'10.0.2.%' IDENTIFIED BY '${mysqlAppPassword}';
GRANT ALL PRIVILEGES ON demodb.* TO 'appuser'@'10.0.2.%';
USE demodb;
...
resource webVm 'Microsoft.Compute/virtualMachines@2023-09-01' = {
  ...
  properties: {
    ...
    osProfile: {
      computerName: '${prefix}-web'
      adminUsername: adminUsername
      adminPassword: adminPassword
      linuxConfiguration: {
        disablePasswordAuthentication: false
      }
      customData: webUserData
    }
    storageProfile: {
      imageReference: {
...
resource appVm 'Microsoft.Compute/virtualMachines@2023-09-01' = {
  ...
  properties: {
    ...
    osProfile: {
      computerName: '${prefix}-app'
      adminUsername: adminUsername
      adminPassword: adminPassword
      linuxConfiguration: {
        disablePasswordAuthentication: false
      }
      customData: appUserData
    }
    storageProfile: {
      imageReference: {
        publisher: ubuntuPublisher
